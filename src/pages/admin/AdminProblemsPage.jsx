...
import { fetchProblems, deleteProblem } from '../../api/admin'
...
export default function AdminProblemsPage() {
  ...
  const load = useCallback(() => {
    fetchProblems({ keyword, page: current, size: 20 })
      .then(setPage)
  ...
  const handleDelete = async (p) => {
    if (!confirm(`"${p.title}" 문제를 정말 삭제할까? (연관 게시글/답변 모두 삭제)`)) return
    try {
          ...
          <tbody>
            {page?.content?.map(p => (
              <tr key={p.id} className="border-t border-gray-100">
                ...
                <td className="px-4 py-3 text-gray-900 font-medium max-w-sm truncate">{p.title}</td>
                <td className="px-4 py-3 text-gray-600">{p.userNickname}</td>
                <td className="px-4 py-3 text-xs text-gray-500">{p.category}</td>
                <td className="px-4 py-3">
                  <span className={`px-2 py-0.5 rounded text-xs font-medium ${STATUS_STYLE[p.status] || 'bg-gray-100 text-gray-600'}`}>
                    {p.status}
                  </span>
                </td>
                <td className="px-4 py-3 text-xs text-gray-500">{p.isPublic ? '공개' : '-'}</td>
